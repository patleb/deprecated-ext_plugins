...
describe RailsAdmin::ApplicationController, type: :controller do
  ...
  describe 'helper method _get_app_name' do
    it 'works by default' do
      expect(controller.send(:_get_app_name)).to eq(['Dummy App', 'Admin'])
    end
    ...
    it 'works for static names' do
      ...
      expect(controller.send(:_get_app_name)).to eq(%w(static value))
    end
    ...
    it 'works for dynamic names in the controller context' do
      ...
      expect(controller.send(:_get_app_name)).to eq(['Dummy App Application', 'Dashboard'])
    end
  end
