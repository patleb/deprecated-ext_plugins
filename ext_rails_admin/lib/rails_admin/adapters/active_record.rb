  ...
  module Adapters
    module ActiveRecord
      ...
      def all(options = {}, scope = nil)
        scope ||= scoped
        scope = scope.left_joins(options[:left_joins]) if options[:left_joins]
        ...
        if options[:page] && options[:per]
          scope = scope.page(options[:page]).per(options[:per])
        end
      ...
      private
        ...
        def build_statement_for_type
          ...
          when :string, :text                           then build_statement_for_string
