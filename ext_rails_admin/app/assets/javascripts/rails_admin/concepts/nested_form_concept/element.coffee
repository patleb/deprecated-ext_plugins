class RailsAdmin.NestedFormConcept::Element
  constants: ->
    TOGGLE_OPENED: 'icon-chevron-down'
    TOGGLE_CLOSED: 'icon-chevron-right'
    ONE: ''
    LAST_BUTTON: ''
    TEMPLATE: 'ID'
    CHILD: ''
  ...
  toggle_nav: ({ force_show = false, force_close = false } = {}) =>
    if force_show || @closed()
      @button_toggle().add_disabled(false) if force_show
      @open_nav()
    else
      @button_toggle().add_disabled() if force_close
      @close_nav()
  ...
  create_nav_tab: =>
    { association_id } = @button_add().data('js')
    { tab_pane_id, object_label } = @append_tab_pane(association_id)
  ...
  append_tab_pane: (association_id) =>
    child_index = $.unique_id(6)
    { template, object_label } = $("#{@TEMPLATE}_#{association_id}").data('js')
    template = template.gsub(///(\[|_)#{@CHILD}_#{association_id}(\]|_)///, "$1#{child_index}$2")
    tab_pane_id = "tab_#{association_id}_#{child_index}"
    @tab_content().append(
      div id: tab_pane_id, class: "#{@TAB_PANE_ADDED} tab-pane fade", ->
  ...
  append_nav_tab: (tab_pane_id, object_label) =>
    nav_tab_wrapper = $(
      li ->
        a href: "##{tab_pane_id}", data: { toggle: 'tab' }, ->
          object_label
    )
