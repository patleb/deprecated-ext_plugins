...
module ActionDispatch
  ...
  #
  #  config.middleware.insert 0, ActionDispatch::TurboDev
  #
  class TurboDev
    def initialize(app)
      @app = app
    ...
    def call(env)
      root = "/assets/"
      is_asset = env['REQUEST_URI'] && env['REQUEST_URI'].starts_with?(root)
      ...
      if (etag = env['HTTP_IF_NONE_MATCH']) && is_asset
        name = env['REQUEST_URI'][(root.length)..-1]
        etag = etag.gsub("\"", "").sub(/^W\//, '')
