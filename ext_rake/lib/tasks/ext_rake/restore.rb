  ...
  class Restore < ExtRake.config.parent_task.constantize
    ...
    def local_storage
      @_local_storage ||= begin
        local_storage = ExtRake.config.backup_local_path.join(directory)
    ...
    def bucket
      @_bucket ||= s3.bucket(ExtRake.config.s3_bucket)
    end

    def s3
      @_s3 ||= Aws::S3::Resource.new(
        access_key_id: ExtRake.config.s3_access_key_id,
        secret_access_key: ExtRake.config.s3_secret_access_key,
        region: ExtRake.config.s3_region
      )
    end
  end
end
