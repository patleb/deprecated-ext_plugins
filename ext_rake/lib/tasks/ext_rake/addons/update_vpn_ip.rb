  ...
  class UpdateVpnIp < ExtRake.config.parent_task.constantize
    ...
    def fetch_record
      @record_set = dns_record_sets.find do |record_set|
        record_set.name.chop == SettingsYml[:vpn_domain] && record_set.type == 'A'
      end
    end
    ...
    def dns_zone
      @_dns_zone ||= dns.list_hosted_zones_by_name.hosted_zones.select{ |zone| zone.name.chop == SettingsYml[:dns_domain] }.first
    end
