...
describe Pundit do
  ...
  before do
    Pundit::PolicyFinder.class_variable_set :@@policies, {}
  end
  ...
  describe ".policy_scope" do
    ...
    it "returns Article if the given policy scope can't be found and ApplicationPolicy::Scope is used" do
      expect(Pundit.policy_scope(user, Article)).to eq(Article)
    end

    it "throw an exception if blank object given" do
  ...
  describe ".policy" do
    ...
    it "returns application policy class if the given policy can't be found" do
      expect(Pundit.policy(user, article)).to be_a(ApplicationPolicy)
      expect(Pundit.policy(user, Article)).to be_a(ApplicationPolicy)
    end

    it "throw an exception if the given policy is nil" do
      expect { Pundit.policy(user, nil) }.to raise_error(Pundit::NotDefinedError)
    end
  ...
  describe "#authorize" do
    ...
    it "does not throw an exception, since ApplicationPolicy is used" do
      expect { controller.authorize(Article) }.not_to raise_error
    end
  ...
  describe "#policy" do
    ...
    it "does not throw an exception, since ApplicationPolicy is used" do
      expect { controller.policy(article) }.not_to raise_error
    end
  ...
  describe "#policy_scope" do
    ...
    it "does not throws an exception, since ApplicationPolicy::Scope is used" do
      expect { controller.policy_scope(Article) }.not_to raise_error
    end
